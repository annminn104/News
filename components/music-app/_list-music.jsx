    ...
    <React.Fragment>
      <div className="max-h-[16rem] overflow-y-auto scroll-smooth">
        {musics.map((music, index) => (
          <div key={music.id}>
            <div
              onClick={() => handleMusics(index)}
              className={`flex gap-2 items-center py-2 border-b border-zinc-300 hover:bg-zinc-100 px-4 hover:text-amber-600 cursor-pointer ${currentMusic.id == music.id ? "!bg-zinc-200" : ""}`}
              role="button"
            >
              <p className={`${currentMusic.id == music.id ? "text-rose-600" : ""}`}>{index + 1}</p>
              ...
              <div>
                <h5 className={`font-semibold ${currentMusic.id == music.id ? "text-rose-600" : ""}`}>{music.title}</h5>
